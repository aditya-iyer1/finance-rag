#!/usr/bin/env python3
"""Validation script for the RAG pipeline (JPMC variant).

Runs a predefined test matrix against the current ChromaDB index and prints
a pass/fail results grid.  Tests JPMorgan Chase 10-K specific questions.

Usage:
    python run_validation_jpmc.py
    python run_validation_jpmc.py --debug      # show retrieval debug output
    python run_validation_jpmc.py --no-color   # plain ASCII table (no ANSI)
"""
...
CATEGORY_A: List[Tuple[str, List[str]]] = [
    ...
    ("What does JPMorgan Chase do?",
     ["banking", "financial", "investment", "commercial"]),
    ("Who is JPMorgan Chase's auditor?",
     ["PricewaterhouseCoopers", "PwC"]),
    ("What are the main risk factors?",
     ["ITEM 1A", "risk", "credit", "market", "regulatory"]),
    ("Where is JPMorgan Chase incorporated?",
     ["Delaware"]),
]
...
CATEGORY_B: List[str] = [
    ...
    "What was JPMorgan Chase's stock price on March 5th?",
]
...
def run_tests(debug: bool, use_color: bool, active_doc_id: str) -> int:
    ...
    # -- header --
    print()
    print(f"{BOLD}RAG Pipeline Validation (JPMC){RESET}")
    print(f"{'=' * 40}")
...
def main():
    parser = argparse.ArgumentParser(description="RAG pipeline validation test matrix (JPMC)")
